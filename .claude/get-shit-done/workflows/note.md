...
- **Global scope**: `D:/Unity/godot-tenten-project/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Unity/godot-tenten-project/.claude/notes/`)
...
2. Glob `D:/Unity/godot-tenten-project/.claude/notes/*.md` (if directory exists) — global notes
...
Global (D:/Unity/godot-tenten-project/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Unity/godot-tenten-project/.claude/notes/` — works in any directory
